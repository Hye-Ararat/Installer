...
(async function setup() {
  ...
  await log(
    "Hye Ararat will automatically be installed on your node through SSH using username/password authentication. Ubuntu is required."
  );
  ...
  try {
    ...
    console.log(
      "❌ I wasn't able to connect to your node. Please make sure the information is correct and try again."
    );
    process.exit(0);
  }
  ...
  if (!osRelease.stdout.includes("Ubuntu")) {
    console.log(
      "❌ System is not compatible! Hye Ararat is only compatible with Ubuntu."
    );
    process.exit(0);
  }
  ...
  channel.on("data", async (d) => {
    ...
    if (d.toString().includes("not in the sudoers file")) {
      await log(
        "❌ Could not escalate to superuser because " +
          username.value +
    ...
    if (
      d
        .toString()
        .includes(
          "You're node has been setup! You can now navigate to it using the URL you specified earlier in your web browser."
        )
    ) {
      console.log(
        "Hye Ararat has been successfully installed and configured! You can now navigate to it using the URL you specified earlier in your web browser."
      );
      process.exit(0);
    }
  });
  ...
  channel.write(
    `cd /usr/lib/ararat && git clone https://github.com/Hye-Ararat/Ararat.git . && echo araratCloned\n`
  );
